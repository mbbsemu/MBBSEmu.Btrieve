...
namespace MBBSEmu.Btrieve
{
    ...
    public class BtrieveFileProcessor : IDisposable
    {
        ...
        protected static readonly Logger<BtrieveFileProcessor> _logger;
        ...
        public void Dispose()
        {
            _logger?.LogInformation($"Closing sqlite DB {FullPath}");
        ...
        public BtrieveFileProcessor(string path, string fileName, int cacheSize)
        {
            ...
            {
                _logger?.LogWarning($"{fullPathDAT} is newer than {fullPathDB}, reconverting the DAT -> DB");
        ...
        {
            _logger?.LogDebug($"Opening sqlite DB {fullPath}");
        ...
        {
            switch (VariableLengthRecords)
            {
                case true when recordData.Length != RecordLength:
                    _logger?.LogDebug($"Updating variable length record of {recordData.Length} bytes into {FullPath}");
                    break;
                case false when recordData.Length != RecordLength:
                    _logger?.LogWarning(
            ...
            {
                _logger?.LogWarning(ex, $"Failed to update record because {ex.SqliteErrorCode} {ex.SqliteExtendedErrorCode} {ex.Message}");
                transaction.Rollback();
            ...
            {
                _logger?.LogWarning(ex, $"Failed to commit during update record because {ex.Message}");
                transaction.Rollback();
                queryResult = 0;
            }
        ...
        private bool InsertAutoincrementValues(SqliteTransaction transaction, byte[] record)
        {
            ...
            try
            {
                if (!reader.Read())
                {
                    _logger?.LogError("Unable to query for MAX auto incremented values, unable to update");
                    return false;
                }
        ...
        public uint Insert(byte[] record, LogLevel logLevel)
        {
            if (VariableLengthRecords && record.Length != RecordLength)
                _logger?.LogDebug($"Inserting variable length record of {record.Length} bytes into {FullPath}");
        ...
        private void PopulateSqliteDataTable(BtrieveFile btrieveFile)
        {
            ...
            foreach (var record in btrieveFile.Records.OrderBy(x => x.Offset))
            {
                ...
                {
                    _logger?.LogError(ex, $"Error importing btrieve data {ex.Message}");
                }
            }
        ...
        {
            _logger?.LogInformation($"Creating sqlite db {fullpath}");
